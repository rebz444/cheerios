...
===========================
Two-stage pipeline producing INDEPENDENT region and cell type columns.

Stage 1 — Region correction (minimal, biological-impossibility only):
  R1: GPe + broad/slow waveform  → CP    (GPe must be fast-spiking)
  R2: RT  + broad waveform       → VAL   (RT is GABAergic fast-spiking)
  R3: STR probe + broad/slow + depth > cortex–STR boundary → CP

Stage 2 — Cell type assignment (waveform + probe + depth):
  MSN       ⟸ broad+slow + STR probe + depth > boundary
  RS        ⟸ broad+slow + (V1 probe OR STR probe shallow)
  FSI       ⟸ narrow waveform (any probe/depth)
  TAN       ⟸ medium PT, tonic FR, STR probe + deep
  high_FR   ⟸ FR > 20 Hz, not FSI
  ambiguous ⟸ otherwise
...
Output columns:
  ...
  region_correction_reason  which rule (R1/R2/R3) fired, if any
...
Outputs:
  RZ_unit_properties_final.csv  (all units, all columns above)
  RZ_str_units.csv              (all units in STR depth zone)
  RZ_str_msn.csv                (STR MSNs — primary for rescaling)
  RZ_str_fsi.csv                (STR FSIs)
  RZ_cortex_units.csv           (units anatomically in motor or visual cortex)
  RZ_v1_cortical.csv            (V1 probe + visual cortex regions)
...
# ══════════════════════════════════════════════════════════════════════════════
print("=" * 70)
print("0g_cell_type_relabeling.py")
...
# ══════════════════════════════════════════════════════════════════════════════
# AUDIT: histology vs depth-gate agreement on STR probe
# ══════════════════════════════════════════════════════════════════════════════
# How often does the depth gate (R3) agree with the histology-based STR label?
# Useful for defending the boundary choice: if "gained" is large, histology was
# under-counting STR; if "lost" is large, the gate is too aggressive.
print("\n" + "-" * 70)
print("AUDIT: histology vs depth on STR probe")
print("-" * 70)

str_probe_df = df[str_probe_mask]
hist_str_mask = str_probe_df["region_acronym"].isin(STR_REGIONS)
depth_deep_mask = str_probe_df["peak_channel_depth"] > CORTEX_STR_BOUNDARY_UM

print(f"\n  STR-probe units claimed as STR by:")
print(f"    histology only            : {hist_str_mask.sum():4d}")
print(f"    depth gate (>{CORTEX_STR_BOUNDARY_UM}µm) only : {depth_deep_mask.sum():4d}")
print(f"    both                      : {(hist_str_mask & depth_deep_mask).sum():4d}")

gained = depth_deep_mask & ~hist_str_mask  # depth says STR, histology disagrees
lost = ~depth_deep_mask & hist_str_mask    # histology says STR, depth disagrees

if gained.sum() > 0:
    print(f"\n  Gained by depth gate (histology said NOT STR, depth >{CORTEX_STR_BOUNDARY_UM}µm): {gained.sum()}")
    for region, count in str_probe_df.loc[gained, "region_acronym"].value_counts().head(8).items():
        print(f"    {region}: {count}")

if lost.sum() > 0:
    print(f"\n  Lost by depth gate (histology said STR, depth ≤{CORTEX_STR_BOUNDARY_UM}µm): {lost.sum()} (likely boundary cases)")
...
# Region group is needed for cortex filtering, so compute it now.
...
# Cortical units: anything anatomically in motor or visual cortex.
# (NOT "V1 probe + STR probe shallow" — that mis-labeled thalamus/hippocampus
# units as cortex when probes overshot.)
cortex_mask = in_cortex_region & ~df["corrected_region"].isin(FIBER_TRACT_REGIONS)
cortex_units = df[cortex_mask].copy()
print(f"  Cortical units (region_group ∈ {{Motor, Visual cortex}}): {len(cortex_units)}")

# V1 cortical: visual cortex on V1 probe (waveform-verified).
# Used by v1_dms_crosscorrelation.py.
...
print(f"  Saved: RZ_v1_cortical.csv ({len(v1_cortical)} units)")
